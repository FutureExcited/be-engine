...
auto ShipCameraController::Update(float deltaTime, BeInput* input) -> void {
    ...
    if (_isCaptured) {
        ...
        float accel = ship.ThrustAccel * std::max(SpeedMul, 0.1f);
        ...
        float speedCap = ship.MaxSpeed * std::max(SpeedMul, 0.1f) * glm::mix(ship.GroundEffectSpeedHigh, ship.GroundEffectSpeedLow, proximity);
